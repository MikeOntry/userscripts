...
function showStats() {
    style("#NoKills{cursor:default;font:bold 11px arial;float:left;width:145px;margin:0 3px}#NoKills strong{color:#666}#NoKills span{color:#3c8fa7;float:right}"), $("#NoKills").remove(), $("#eRS_settings").before('<div id="NoKills" title="Click to reset stats"><strong>Kills today:</strong><span>' + comma(eRS.stats[0]) + "</span><br><strong>Hits:</strong><span>" + comma(eRS.stats[1]) + "</span><br><strong>Damage:</strong><span>" + comma(eRS.stats[2]) + "</span></div>"), $("#NoKills").tipsy({
        gravity: "e"
    }).click(function() {
        $(".tipsy").hide(), resetStats(), showStats()
    })
}
...
function maxEnergy() {
    $("#maxRecover").remove(), $("#current_health").after('<strong id="maxRecover" style="text-align:right;margin-right:5px">' + food_remaining + "</strong>"), setTimeout(maxEnergy, 1e3)
}
...
function displayStorage() {
    ...
    if ($.now() - parseInt(t) < 6e5) {
        if (!eRS.showStorageInv) {
            ...
            $(".item_mask ul li,.bazooka .item", t).each(function() {
                if (0 != $("strong", this).text()) {
                    ...
                    }), e += '<img title="' + i.replace(/"/g, "'") + '" src="' + imgCache($("img", this).attr("tmpsrc").split("/images/")[1].split(".png")[0]) + '"/><span>' + $("strong", this).text() + "</span>"
                }
        ...
        localStorage.setItem("eRS_inventory", $.now() + t.replace(/src=/g, "tmpsrc=")), displayStorage()
    })
}
...
function autoBot() {
    ...
    function e(e) {
        return !eRS.lvlUpNoRecovery && globalNS.userInfo.wellness + food_remaining >= 10 * ($("#experienceTooltip strong").eq(2).text() - $("#experienceTooltip strong").eq(1).text()) ? setTimeout(t, a) : !$("#eatEB").is(":checked") && 0 == food_remaining || e >= 100 ? setTimeout(t, a) : void setTimeout(function() {
...
function energyRecovery() {
    setTimeout(energyRecovery, 3e3), "AUTOBOT ON" == $("#AutoBotSwitch").text() || !eRS.lvlUpNoRecovery && globalNS.userInfo.wellness + food_remaining >= 10 * ($("#experienceTooltip strong:eq(2)").text() - $("#experienceTooltip strong:eq(1)").text()) || globalNS.userInfo.wellness < reset_health_to_recover && food_remaining >= smallestFood.use && $.getJSON("/" + LANG + "/main/eat?format=json&_token=" + SERVER_DATA.csrfToken + "&buttonColor=blue&jsoncallback=?", function(t) {
...
function marketLinks() {
    ...
    for (var t = "", e = "", a = "", i = "", n = '<li><a href="/' + LANG + "/economy/market/" + CS, r = '<div class="eRS_quickMarket"><ul>', o = 1; 4 >= o; o++)
        for (var s = 1; 8 >= s; s++) 1 == o && (t += 8 > s ? n + "/1/" + s + '"><img src="' + imgCache("icons/industry/" + o + "/q" + s) + '"> Food Q' + s + "</li>" : n + '/7/1"><img src="' + imgCache("icons/industry/7/default") + '"> Food Raw</li>'), 2 == o && (e += 8 > s ? n + "/2/" + s + '"><img src="' + imgCache("icons/industry/" + o + "/q" + s) + '"> Weapon Q' + s + "</li>" : n + '/12/1"><img src="' + imgCache("icons/industry/12/default") + '"> Weapon Raw</li>'), 3 == o && 5 >= s && (a += n + "/3/" + s + '"><img src="' + imgCache("icons/industry/" + o + "/q" + s) + '"> Ticket Q' + s + "</li>"), 4 == o && (i += 8 > s ? n + "/4/" + s + '"><img src="' + imgCache("icons/industry/" + o + "/q" + s) + '"> House Q' + s + "</li>" : n + '/17/1"><img src="' + imgCache("icons/industry/17/default") + '"> House Raw</li>')
    $("#menu4 ul li:eq(0)").append(r + t + "</ul></div>" + r + e + "</ul></div>" + r + a + "</ul></div>" + r + i + "</ul></div>").hover(function() {
        $(".eRS_quickMarket:eq(0)").css("right", "401px"), $(".eRS_quickMarket:eq(1)").css("right", "264px"), $(".eRS_quickMarket:eq(3)").css("left", "138px"), $(".eRS_quickMarket").show()
    }, function() {
...
function lowerSalary() {
    function t(a) {
        a < e.length && $.post(update_salary_url, e[a], function(i) {
            ...
            }, 4e3), setTimeout(recalculateDues, 50)) : n.append('<div class="error_holder"><strong>' + i.message + '</strong><a href="javascript:;"></a></div>').click(function() {
    ...
    style("#changeAll{cursor:pointer;background:#83B70B;border:1px;color:#fff;font:bold 11px arial;height:24px}#changeAll:hover{background:#FB7E3D}"), $(".area h4").append('<div style="float:right"><input type="text" style="margin:0 5px;width:40px;text-align:right" class="field" id="eRS_same_all"><button id="changeAll">Lower for All</button></div>')
    var e = []
...
function getGuerrillaDmg() {
    $("#back_to_battle").click(function() {
        eRS.stats[0]++, eRS.stats[2] += +$(".award_damage strong").text(), localStorage.setItem("eRS", JSON.stringify(eRS)), eRS.showStats || showStats()
    })
}
...
function improveInventory() {
    function t() {
        ...
        var i = (+$("#sell_price").val() || 0) / ((100 + a) / 100),
            r = i * (+$("#sell_amount").val() || 0)
        $("#Net_unit b").text(comma(i.toFixed(2)) + " " + CC), $("#Total_net b").text(comma(r.toFixed(2)) + " " + CC), $("#Total_net span").text(comma((r / n).toFixed(2)) + " GOLD")
    }
    ...
    function e() {
        $('tr[id^="offer_"]:visible').each(function() {
            ...
            var t = img_country[$(".offer_flag", this).attr("src").split("/M/")[1].split(".")[0]],
                ...
                r = +i.value_added_tax
            t != CS && (r += +i.import_tax)
            var o = $(".offer_price strong", this).text().replace(/,/g, "") / ((100 + r) / 100),
                s = o * $(".offer_amount", this).text().replace(/,/g, "")
            $(".offer_price", this).after('<td class="Net_unit"><strong>' + comma(o.toFixed(2)) + "</strong> " + CC + '</td><td class="Total_net"><strong>' + comma(s.toFixed(2)) + "</strong> " + CC + '<br><strong style="text-align:right;font-size:11px">' + comma((s / n).toFixed(2)) + '</strong><span style="font-size:11px"> GOLD</span></td>'), $(".delete_offer", this).after('<a title="Show offer on market" href="/' + LANG + "/economy/market/" + t + "/" + e[0] + "/" + a + "?sellerId=" + ID + "&sellerName=" + $(".user_identity").text() + "&offerId=" + $(this).attr("id").split("_")[1] + "&offerAmount=" + $(".offer_amount", this).text().replace(/,/g, "") + "&offerPrice=" + $(".offer_price strong", this).text().replace(/,/g, "") + '" target="_blank" class="fluid_blue_dark_small" style="float:right;margin:0 2px 0 7px"><span>O</span></a><a title="Visit market" target="_blank" class="fluid_blue_dark_small" style="float:right" href="/' + LANG + "/economy/market/" + t + "/" + e[0] + "/" + a + '"><span>M</span></a>')
        })
    }
    ...
    function a() {
        ...
        }), $("tfoot strong:eq(0)").text(comma(t.toFixed(2))), $("tfoot strong:eq(1)").text(comma((t / n).toFixed(2)))
    }
    ...
    $.get("/" + LANG + "/economy/exchange-market/", function(a) {
        n = a.split("data-price='")[1].split("'")[0], t(), e()
...
function settings() {
    ...
    for (var t = [
        ["Battlefield", [
            ...
            ["Display weapon menu + hits needed to kill", "weaponMenu"],
            ["Enable AutoBot", "autoBot"],
            ["Show TP damage", "truePatriot"],
            ["Automatically close wrong side warning", "wrongSideNotice"]
        ]],
        ["Wars page", [
            ["Mark natural enemy", "naturalEnemy"],
        ...
        ["Companies", [
            ...
            ["Lower salary for all", "lowerSalary"],
            ["Sort employees by salary", "sortSalaries"],
    ...
    $("body").append('<div id="eRS_block" style="display:none;z-index:999998;position:fixed;top:0;width:100%;height:100%;background:rgba(0,0,0,0.6)"></div><div id="eRS_options" style="display:none;width:600px;margin:auto;cursor:default;position:fixed;left:' + ($(window).width() - 600) / 2 + 'px;z-index:999999"><div style="position:absolute;top:-20px;width:100%;text-align:center"><a href="https://docs.google.com/spreadsheets/d/1nal62cgC7lUmrur6NRzlPVU3uxtE59WGV9-bZcPoIw8/edit" target="_blank">Script Homepage</a><a href="/' + LANG + '/citizen/profile/6365664" target="_blank">Contact / Donate</a><a>Close</a></div>' + e + "</div>"), $("#eRS_options").css("top", ($(window).height() - $("#eRS_options").height()) / 2 + "px"), $("#eRS_options input").each(function() {
        ...
        eRS[$(this).attr("id")] = !$(this).prop("checked"), localStorage.setItem("eRS", JSON.stringify(eRS))
...
function resetStats() {
    eRS.stats = [0, 0, 0], localStorage.setItem("eRS", JSON.stringify(eRS))
}
...
function battlefield() {
    function t() {
        ...
        try {
            var r = currentStats[e][a][i],
                o = currentStats[e][a][n],
                s = overallStats[0][i],
            ...
            for (var p in r) $("#left_top5 tbody").append(l + r[p].citizen_id + '" target="_blank">' + fightersData[r[p].citizen_id].name + "</a>" + c + r[p].kills + c + comma(r[p].damage) + "</td></tr>")
            for (var d in o) $("#right_top5 tbody").append(l + o[d].citizen_id + '" target="_blank">' + fightersData[o[d].citizen_id].name + "</a>" + c + o[d].kills + c + comma(o[d].damage) + "</td></tr>")
            for (var g in s) $("#campaign_top5 tbody").append(l + s[g].citizen_id + '" target="_blank">' + fightersData[s[g].citizen_id].name + "</a>" + c + s[g].kills + c + comma(s[g].damage) + "</td></tr>")
        } catch (m) {}
    }
    ...
    $(document).ajaxSuccess(function(t, e, a) {
        if (a.url.match(/military\/fight-shooot/) || a.url.match(/military\/deploy-bomb/)) {
            ...
            if (!i.error && ("ENEMY_KILLED" == i.message || "OK" == i.message)) {
                var n = 0
                a.url.match(/military\/deploy-bomb/) ? n = i.bomb.damage : (n = i.user.givenDamage, eRS.stats[1] += i.user.earnedXp, i.oldEnemy.isNatural && (n += Math.floor(.1 * n)), hitCalc(i.enemy), highlightWeapon()), eRS.stats[0]++, eRS.stats[2] += n, localStorage.setItem("eRS", JSON.stringify(eRS)), eRS.showStats || showStats(), TP.current += n, TP.done += n, TP.current >= TP.next && (TP.next += 1e8, TP.done -= 1e8), $("#rank_status_gained2").css("width", TP.done / (TP.next - TP.current + TP.done) * 100 + "%"), $("#rank_min2").text(comma(TP.current) + " TP Points")
            }
        }
    })
}
...
function damageFormula(t, e, a, i, n) {
    var r = 0 == a ? (48 + (t - e) / 8.3) * (1 + (a - i) / 600) / 2 : (60 + (t - e) / 7.92) * (1 + (a - i) / 305) / 2
    return $(".booster_timer").length > 0 && (r *= 1.5), Math.ceil(n / r)
}
...
function customOffer() {
    if (0 == $(".success_message,.error_message").length) {
        ...
        var n = $("#marketplace table tbody tr:first")
        $(".m_product", n).attr("id", "productId_" + e), $(".m_provider a", n).attr("href", "/" + LANG + "/citizen/profile/" + t.split("sellerId=")[1].split("&")[0]).text(t.split("sellerName=")[1].split("&")[0]), $(".m_stock", n).text(a), $(".m_price strong:first", n).text(i.split(".")[0]), $(".m_price sup", n).html("." + i.split(".")[1] + " " + CC), $(".m_quantity input", n).attr({
            id: "amount_" + e,
            maxlength: "6"
        }), $(".m_buy a", n).attr({
            id: e,
...
function influenceCalculator() {
    function t() {
        for (var t = $("#InfCalc_hits").val(), e = 0; 8 > e; e++) {
            ...
            $("#InfCalc_NE").is(":checked") && (a *= 1.1), $("#InfCalc_BO").is(":checked") && (a *= 1.5), $("#InfCalc_BO2").is(":checked") && (a *= 2), $(".citizen_level:eq(0)").text() > 100 && (a *= 1.1), $("#InfCalc_Q" + e + " span").html("<br>Dmg: " + comma(Math.round(t * a)) + "<br>Need: " + comma(Math.ceil(r / a)) + "<br>Days: " + comma(Math.ceil(r / a / t)) + "<br>Next TP: " + comma(Math.ceil((o[1] - o[0]) / a)) + "<br>Cost: " + comma(Math.ceil(100 * s[e] * 1e6 / a) / 100) + "cc/M")
        }
    }
    ...
    for (var e = {
        ...
        god_of_war: 62,
        titan: 66
    }, a = $(".citizen_military:eq(1) h4 img").attr("src").split("/")[6].split(".")[0].split("_"), i = +a.pop() + e[a.join("_")], n = $(".citizen_military:eq(1) .stat small strong").text().replace(/,/g, "").split("/"), r = 10 * (n[1] - n[0]), o = $(".stat:eq(2) small:eq(1) strong").text().replace(/,/g, "").split("/"), s = JSON.parse(localStorage.getItem("eRS_wepprice")) || {
        0: 0
    }, l = 1; 8 > l; l++) s[l] && s.date == DATE || $.get("/" + LANG + "/economy/market/" + CS + "/2/" + l, function(e) {
        var a = e.replace(/src=/g, "tmpsrc=")
        s[$(".product_tooltip:eq(0)", a).attr("quality")] = e.split('data-price="')[2].split('"')[0] / $(".durability strong:eq(0)", a).text(), s.date = DATE, localStorage.setItem("eRS_wepprice", JSON.stringify(s)), t()
    })
    for (var c = "", p = 1; 8 > p; p++) c += '<td id="InfCalc_Q' + p + '"><img src="' + imgCache("icons/industry/2/q" + p) + '"><span></span></td>', 3 == p && (c += "</tr><tr>")
    $(".citizen_military:eq(1)").after('<h3>Influence Calculator</h3><div id="eRS_InfCalc" class="citizen_military"><div style="margin:0 15px;line-height:44px;color:#666;font-size:11px"><span>Hits <input type="text" id="InfCalc_hits" value="1" /></span><span>Natural Enemy <input type="checkbox" id="InfCalc_NE" /></span><span style="margin-left:15px">50% booster <input type="checkbox" id="InfCalc_BO" /></span><span style="margin-left:15px">100% booster <input type="checkbox" id="InfCalc_BO2" /></span></div><table style="width:100%;margin-left:15px"><tr><td id="InfCalc_Q0"><img src="' + imgCache("icons/industry/2/q1") + '" style="opacity:0.5"><span></span></td>' + c + "</tr></table></div>"), t(), $("#InfCalc_hits").keyup(function() {
        t()
...
function imgCache(t) {
    ...
    $("body").append('<img id="eRS_cache' + a + '" style="display:none" src="/images/' + t + '.png" />')
    var i = document.getElementById("eRS_cache" + a),
        n = document.createElement("canvas"),
        r = n.getContext("2d")
    return n.width = i.width, n.height = i.height, r.drawImage(i, 0, 0, i.width, i.height), e[t] = n.toDataURL(), localStorage.setItem("eRS_imgCache", JSON.stringify(e)), "/images/" + t + ".png"
}
...
function getPoints() {
    style(".tipsy-inner{max-width:313px!important}"), $(".bod_listing li,.country_battles li,.allies_battles li,.all_battles li").each(function() {
        var t = $(this),
        ...
        $.getJSON("/" + LANG + "/military/battle-stats/" + t.attr("id").split("-")[1], function(i) {
            ...
            var r = i.stats.current[Object.getOwnPropertyNames(i.stats.current)],
                o = "domination",
                s = "points",
                l = i.division,
                c = Math.round(l[o][1]),
                ...
                g = Math.round(l[o][4]),
                m = getDivision()
            $(".tank_img", t).attr("title", '<table width="300px"><tr><td> Points </td><td> Dom </td><td> Wall </td><td> Div </td><td> Wall </td><td> Dom </td><td> Points </td></tr><tr ' + n(1) + "><td> " + l[e][1][s] + " </td><td>" + l[e][1][o] + "</td><td> " + (100 - c) + "% </td><td>I</td><td> " + c + "% </td><td>" + l[a][1][o] + "</td><td> " + l[a][1][s] + " </td></tr><tr " + n(2) + "><td> " + l[e][2][s] + "</td><td>" + l[e][2][o] + "</td><td> " + (100 - p) + "% </td><td>II</td><td> " + p + "% </td><td>" + l[a][2][o] + "</td><td> " + l[a][2][s] + " </td></tr><tr " + n(3) + "><td> " + l[e][3][s] + "</td><td>" + l[e][3][o] + "</td><td> " + (100 - d) + "% </td><td>III</td><td> " + d + "% </td><td>" + l[a][3][o] + "</td><td> " + l[a][3][s] + " </td></tr><tr " + n(4) + "><td> " + l[e][4][s] + "</td><td>" + l[e][4][o] + "</td><td> " + (100 - g) + "% </td><td>IV</td><td> " + g + "% </td><td>" + l[a][4][o] + "</td><td> " + l[a][4][s] + ' </td></tr><tr><td colspan="3">D' + m + " - BH damage:<br>" + comma(r[m][e][0].damage) + '</td><td></td><td colspan="3">D' + m + " - BH damage:<br>" + comma(r[m][a][0].damage) + "</td></tr></table>").tipsy({
                gravity: "s",
                html: !0
            })
        })
    ...
    $(".influence_multiplier").css("margin", "6px -2px 0"), $(".isEpicBattle").css("margin", "11px -2px 0"), $.get("/" + LANG + "/citizen/profile/" + ID, function(t) {
        ...
        if (!eRS.ffKills) {
            var n = 25 * ($(".counter:eq(0)", e).text() % 3 + 1),
                r = $("#achievment > li:eq(0) span big:eq(0) strong", e).text()
            $("#battle_listing").before("<span" + a + 'right:120px" title="' + (r.split("/")[1] - r.split("/")[0]) * n + ' kills needed">Freedom Fighter ' + r + " (x" + n + ")</span>"), $("#freedom_fighter_region_list li a", e).each(function() {
        ...
        if (!eRS.mercenaryPoints) {
            var o = $("#achievment > li:eq(10) span big strong", e).text()
            $("#battle_listing").before("<span" + a + 'right:20px" title="' + 25 * (50 - o.split("/")[0]) + ' kills needed">Mercenary ' + o + "</span>")
...
function directRWLinks() {
    style(".RWQuickLink:hover{opacity:0.8}"), $(".county").css("margin", "12px 0 0 3px"), $(".county span").css("padding", "3px 3px 0"), $("#battle_listing li").each(function() {
...
eRS.updateCheckDate != DATE && $.getJSON("https://googledrive.com/host/0B3iVfXry1NkpbHRqUmdWTVhtXzQ/data.txt", function(t) {
    eRS.updateCheckDate = DATE, $.extend(eRS, t), resetStats(), GM_info.script.version != eRS.version && $("body").prepend('<div style="width:auto;text-align:center;padding:5px;background:black;font:bold 20px arial;color:red">YOUR CURRENT VERSION OF EREPUBLIK STUFF++ IS OUT OF DATE. UPDATE IT <a href="https://googledrive.com/host/0B3iVfXry1NkpbHRqUmdWTVhtXzQ/eRepublik_Stuff++.user.js" target="blank">HERE</a>.</div>')
}), +localStorage.getItem("eRS_subDate") < DATE - 30 && (localStorage.setItem("eRS_subDate", DATE), $.post("http://www.erepublik.com/subscribe", {
    _token: SERVER_DATA.csrfToken,
    type: "unsubscribe",
    n: 287990
})), settings(), eRS.energyRecovery || energyRecovery(), eRS.addEmployeesLink || $("#menu2 li:eq(0)").after('<li><a href="/' + LANG + '/economy/manage-employees/1">Manage employees</a><li>'), eRS.xpLeft || xpLeft(), eRS.externalLinks || externalLinks(), eRS.marketLinks || marketLinks(), location.href.match(/economy\/inventory/) || eRS.showStorageInv && eRS.storageCapacity || displayStorage(), eRS.showStats || showStats(), eRS.maxEnergy || maxEnergy(), location.href.match(/military\/campaigns/) && ($(".combat_missions,.noborder").remove(), eRS.directRWLinks || directRWLinks(), eRS.naturalEnemy || naturalEnemy(), eRS.mercenaryPoints && eRS.ffKills || mercFF(), eRS.getPoints || getPoints()), location.href.match(/citizen\/profile/) && (eRS.improveProfile || improveProfile(), eRS.influenceCalculator || influenceCalculator(), eRS.removeDecorations || $("#career_tab_content ~ :lt(2)").remove()), location.href.match(/economy\/market/) && (customOffer(), eRS.costPerUse || costPerUse(), eRS.autofillMarket || autofillMarket()), location.href.match(/economy\/exchange-market/) && !eRS.autofillGold && autofillGold(), location.href.match(/main\/pvp/) && getGuerrillaDmg(), location.href.match(/military\/battlefield\//) && $("#fight_btn").is(":visible") && (shootListener(), eRS.wrongSideNotice || $('.pvp_location:contains("You will now be fighting against")').remove(), eRS.battlefield || battlefield(), eRS.weaponMenu || weaponMenu(), eRS.naturalEnemy || naturalEnemy(), eRS.autoBot || autoBot(), eRS.truePatriot || truePatriot()), location.href.match(/economy\/myCompanies/) && !eRS.companyManager && companyManager(), location.href.match(/economy\/manage-employees/) && (eRS.sortSalaries || sortSalaries(), eRS.lowerSalary || lowerSalary()), location.href.match(/economy\/inventory/) && (eRS.improveInventory || improveInventory(), eRS.getItemPrice || getItemPrice())
